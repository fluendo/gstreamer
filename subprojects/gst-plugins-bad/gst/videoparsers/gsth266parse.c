...
{
  /* The H266 parameters are very big and may cause stack overflow.
   * Just cache them to avoid frequently malloc and free. */
  GstH266VPS *cached_vps;
  GstH266SPS *cached_sps;
  GstH266PPS *cached_pps;
  GstH266APS *cached_aps;
  GstH266PicHdr *cached_ph;
};
/* *INDENT-ON* */
...
gst_h266_parse_init (GstH266Parse * h266parse)
{
  ...
  h266parse->priv->cached_vps = g_malloc (sizeof (GstH266VPS));
  h266parse->priv->cached_sps = g_malloc (sizeof (GstH266SPS));
  h266parse->priv->cached_pps = g_malloc (sizeof (GstH266PPS));
  h266parse->priv->cached_aps = g_malloc (sizeof (GstH266APS));
  h266parse->priv->cached_ph = g_malloc (sizeof (GstH266PicHdr));
...
gst_h266_parse_finalize (GObject * object)
{
  ...
  g_free (h266parse->priv->cached_vps);
  g_free (h266parse->priv->cached_sps);
  g_free (h266parse->priv->cached_pps);
  g_free (h266parse->priv->cached_aps);
  g_free (h266parse->priv->cached_ph);
...
{
  GstH266VPS *vps = h266parse->priv->cached_vps;
  GstH266SPS *sps = h266parse->priv->cached_sps;
  GstH266PPS *pps = h266parse->priv->cached_pps;
  GstH266APS *aps = h266parse->priv->cached_aps;
  GstH266PicHdr *ph = h266parse->priv->cached_ph;
  guint nal_type;
  GstH266Parser *nalparser = h266parse->nalparser;
  ...
  memset (vps, 0, sizeof (GstH266VPS));
  memset (sps, 0, sizeof (GstH266SPS));
  memset (pps, 0, sizeof (GstH266PPS));
  memset (aps, 0, sizeof (GstH266APS));
  memset (ph, 0, sizeof (GstH266PicHdr));

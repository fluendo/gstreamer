 ...
 * @metadata_application_format (out): the metadata_application_format
 * @metadata_format (out): the metadata_format
 * @metadata_format_identifier (out): the metadata_format_identifier (equivalent to 
 * format_identifier in registration_descriptor), only valid if metadata_format is 0xFF.
 * @metadata_service_id (out): the metadata service to which this descriptor applies.
 * @decoder_config_flags (out): whether and how decoder config information is conveyed,
 * as shown in ISO/IEC-13818-1:2018 Table 2-88.
 * @dsm_cc_flag (out): true if the associated stream is carried in ISO/IEC 13818-6 data
 * or object carousel.
 *
 * Extracts the metadata descriptor from @descriptor.
 * 
...
gst_mpegts_descriptor_parse_metadata (const GstMpegtsDescriptor * descriptor,
    guint16 * metadata_application_format, guint8 * metadata_format,
    guint32 * metadata_format_identifier, guint8 * metadata_service_id,
    guint8 * decoder_config_flags, gboolean * dsm_cc_flag)
{
  guint8 *data;
  guint8 flag;

  g_return_val_if_fail (descriptor != NULL
      && metadata_application_format != NULL && metadata_format != NULL
      && metadata_format_identifier != NULL && metadata_service_id != NULL
      && decoder_config_flags != NULL && dsm_cc_flag != NULL, FALSE);
  __common_desc_checks (descriptor, GST_MTS_DESC_METADATA, 5, FALSE);

  data = (guint8 *) descriptor->data + 2;
  *metadata_application_format = GST_READ_UINT16_BE (data);
  data += 2;
  if (*metadata_application_format == 0xFFFF) {
    // skip over metadata_application_format_identifier if it is provided
    data += 4;
  }
  *metadata_format = *data;
  data += 1;
  if (*metadata_format == 0xFF) {
    *metadata_format_identifier = GST_READ_UINT32_BE (data);
    data += 4;
  }
  *metadata_service_id = *data;
  data += 1;
  flag = *data;
  *decoder_config_flags = flag >> 5;
  *dsm_cc_flag = (flag & 0x10);

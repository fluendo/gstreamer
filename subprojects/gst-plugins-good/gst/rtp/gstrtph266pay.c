...
#define NALU_IS_PS(nalu) \
  (((nalu)->type >= GST_H266_NAL_VPS) && ((nalu)->type <= GST_H266_NAL_SUFFIX_APS))
#define NALU_IS_VCL(nalu) \
  (((nalu)->type >= GST_H266_NAL_SLICE_TRAIL) && ((nalu)->type <= GST_H266_NAL_RSV_IRAP_11))
#define NALU_IS_IDR(nalu) \
  (((nalu)->type == GST_H266_NAL_SLICE_RADL) || ((nalu)->type == GST_H266_NAL_SLICE_IDR_N_LP))
...
typedef struct
{
  ...
  GstH266NalUnitType type;      // nal_unit_type
...
static GstFlowReturn _handle_byte_stream_buffer (GstRtpH266Pay * rtph266pay,
    GstBuffer * buffer);
...
static guint _next_start_code (const guint8 * data, guint size);
...
gst_rtp_h266_pay_init (GstRtpH266Pay * rtph266pay)
{
  ...
  rtph266pay->stream_format = STREAM_FORMAT_UNKNOWN;
...
_set_caps (GstRTPBasePayload * rtpbasepay, GstCaps * caps)
{
  ...
  stream_format_str = gst_structure_get_string (s, "stream-format");
  if (stream_format_str) {
    if (g_str_equal (stream_format_str, "byte-stream")) {
      rtph266pay->stream_format = STREAM_FORMAT_BYTE_STREAM;
    } else {
      rtph266pay->stream_format = STREAM_FORMAT_UNKNOWN;
    }
  }
...
_handle_buffer (GstRTPBasePayload * rtpbasepay, GstBuffer * buffer)
{
  ...
  switch (rtph266pay->stream_format) {
    case STREAM_FORMAT_BYTE_STREAM:
      return _handle_byte_stream_buffer (rtph266pay, buffer);
    default:
      GST_ELEMENT_ERROR (rtph266pay, STREAM, NOT_IMPLEMENTED,
          ("Stream format not supported"), ("Stream format not supported"));
...
static GstFlowReturn
_handle_byte_stream_buffer (GstRtpH266Pay * rtph266pay, GstBuffer * buffer)
{
  guint8 *current_nalu;
  GstMapInfo info;
  ...
  guint next_sc = _next_start_code (info.data, info.size);
  if (next_sc == info.size) {
    GST_WARNING_OBJECT (rtph266pay, "No NALU found");
    goto beach;
  }
  current_nalu = info.data + next_sc;

  // For each NALU
  do {
    guint current_offset = current_nalu - info.data;
    guint remaining = info.size - current_offset;
    guint next_sc =
        _next_start_code (current_nalu + NALU_SC_LEN, remaining - NALU_SC_LEN);
    guint8 *next_nalu = current_nalu + NALU_SC_LEN + next_sc;

    gsize nalu_len = next_nalu - current_nalu;
    current_nalu = next_nalu;
    if (nalu_len <= NALU_SC_LEN) {
      GST_WARNING_OBJECT (rtph266pay, "NALU too small, skipping it");
      continue;
    }

    // Remove the start code
    nalu_len -= NALU_SC_LEN;
    gsize nalu_offset = current_offset + NALU_SC_LEN;

    GST_DEBUG_OBJECT (rtph266pay, "Found NALU");
    GST_MEMDUMP_OBJECT (rtph266pay, "NALU", info.data + nalu_offset, 10);
    GstBuffer *nalu_buf =
        gst_buffer_copy_region (buffer, GST_BUFFER_COPY_MEMORY, nalu_offset,
        nalu_len);
    // nalu_buf will never be aligned with an incoming buffer, therefore
    // gst_buffer_copy_region() will never set its timestamps
    GST_BUFFER_PTS (nalu_buf) = GST_BUFFER_PTS (buffer);
    GST_BUFFER_DTS (nalu_buf) = GST_BUFFER_DTS (buffer);

    _process_nalu (rtph266pay, nalu_buf);
  } while (current_nalu < (info.data + info.size));
...
_set_au_boundaries (GstRtpH266Pay * rtph266pay)
{
  ...
  for (GList * l = head; l != NULL; l = l->next) {
    ...
    gboolean aud = nalu->type == GST_H266_NAL_AUD;
    gboolean new_ts = (prev_pts != pts) || (prev_dts != dts);

    nalu->au_start = aud || new_ts;
...
{
  switch (nalu->type) {
    case GST_H266_NAL_RSV_VCL_4:
    case GST_H266_NAL_RSV_VCL_5:
    case GST_H266_NAL_RSV_VCL_6:
    case GST_H266_NAL_RSV_IRAP_11:
    case GST_H266_NAL_RSV_NVCL_26:
    case GST_H266_NAL_RSV_NVCL_27:
      return TRUE;
    default:
      return FALSE;
  }
...
// From commit: 2ad5efaf282b0a3f70000be5205a67613655a1ca
static guint
_next_start_code (const guint8 * data, guint size)
{

...
not_active:
  {
    ...
    /* unselected pad, perform fallback alloc when asked */
...
gst_selector_pad_chain (GstPad * pad, GstBuffer * buf)
{
  ...
  if (G_UNLIKELY (sel->pending_close) && G_UNLIKELY (selpad->segment_pending)) {
  ...
  GST_LOG_OBJECT (pad, "Buffer forwarded %p result=%d", buf, res);
